...
import { type Agent, type AgentCatalog } from "@capyfin/contracts";
  ...
  const defaultAgentId = (() => {
    const candidate =
      typeof record.defaultAgentId === "string"
        ...
        : DEFAULT_AGENT_ID;
    return agents[candidate] ? candidate : DEFAULT_AGENT_ID;
  })();
...
export class AgentMetadataStoreService {
  ...
  async ensureDefaultAgent(): Promise<Agent> {
    ...
    const existing = store.agents[DEFAULT_AGENT_ID];
    if (existing) {
      if (store.defaultAgentId !== DEFAULT_AGENT_ID) {
        store.defaultAgentId = DEFAULT_AGENT_ID;
        await saveStore(this.#paths.metadataPath, store);
      }
      return toAgentRecord(this.#paths, store, existing);
    }
