...
async function main(): Promise<void> {
  const token = process.env.CAPYFIN_GATEWAY_TOKEN?.trim();
  const port = Number.parseInt(process.env.CAPYFIN_GATEWAY_PORT ?? "", 10);
  if (!token) {
    throw new Error("Missing CAPYFIN_GATEWAY_TOKEN.");
  }
  if (!Number.isInteger(port) || port <= 0 || port > 65_535) {
    throw new Error("Missing or invalid CAPYFIN_GATEWAY_PORT.");
  }
