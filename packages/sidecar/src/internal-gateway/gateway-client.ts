...
/** Regex matching an 8-char hex prefix (auto-generated UUID labels). */
const UUID_LABEL_RE = /^[0-9a-f]{8}(\s*\(|$)/i;

export function isAutoGeneratedLabel(value: string | undefined): boolean {
  const trimmed = value?.trim();
  return !trimmed || UUID_LABEL_RE.test(trimmed);
}

function resolveSessionLabel(row: GatewaySessionRow): string | undefined {
  // Prefer a human-set label (skip auto-generated UUID labels)
  const label = row.label?.trim();
  if (label && !UUID_LABEL_RE.test(label)) return label;
  const displayName = row.displayName?.trim();
  if (displayName && !UUID_LABEL_RE.test(displayName)) return displayName;
  // Fall back to the gateway's derived title (typically the first user message)
  return row.derivedTitle?.trim() ?? undefined;
}
...
function toAgentSession(params: {
  ...
  const session: AgentSession & { model?: string } = {
    ...
    createdAt: updatedAt,
    id: sessionId,
    label: resolveSessionLabel(params.row),
...
export class EmbeddedGatewayClient {
  ...
  async listSessions(agentId?: string): Promise<AgentSessionList> {
    ...
    // Enrich unnamed sessions with a preview derived from the first user message.
    const unnamed = sessions.filter((s) => !s.label);
    if (unnamed.length > 0) {
      await Promise.allSettled(
        unnamed.slice(0, 15).map(async (session) => {
          try {
            const history = await this.request<GatewayChatHistoryResult>(
              "chat.history",
              { limit: 10, sessionKey: session.sessionKey },
            );
            const firstUserMsg = history.messages
              .map((m) => toChatTranscriptMessage(m))
              .find((m): m is ChatTranscriptMessage => m?.role === "user");
            ...
            // Best-effort — leave the session unnamed
          }
        }),
      );
    }
