...
void test("listBundledSkills returns the bundled financial skill names", async () => {
  ...
  assert.ok(
    skills.length >= 5,
    `Expected at least 5 skills, got ${String(skills.length)}`,
  );
  assert.ok(skills.includes("stock-analysis"), "Missing stock-analysis skill");
  assert.ok(
    skills.includes("portfolio-analyzer"),
    "Missing portfolio-analyzer skill",
  );
  assert.ok(
    skills.includes("earnings-summary"),
    "Missing earnings-summary skill",
  );
  assert.ok(
    skills.includes("market-screener"),
    "Missing market-screener skill",
  );
  assert.ok(
    skills.includes("market-overview"),
    "Missing market-overview skill",
  );
});
...
void test("installBundledSkills copies skills to the workspace", async () => {
  ...
  try {
    ...
    assert.ok(
      installed.length >= 5,
      `Expected at least 5 installed, got ${String(installed.length)}`,
    );
    ...
    assert.ok(
      entries.includes("portfolio-analyzer"),
      "portfolio-analyzer not copied",
    );
