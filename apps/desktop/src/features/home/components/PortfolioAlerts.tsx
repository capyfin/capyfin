...
export function PortfolioAlerts({ alerts, embedded }: PortfolioAlertsProps) {
  if (alerts.length === 0) return null;

  const content = (
    <div className={embedded ? "px-1 py-1" : "space-y-1"}>
      {!embedded && null}
      {embedded && (
        <p className="px-2 pt-1.5 pb-0.5 text-[11px] font-medium uppercase tracking-[0.1em] text-muted-foreground/50">
          Portfolio Alerts
        </p>
      )}
