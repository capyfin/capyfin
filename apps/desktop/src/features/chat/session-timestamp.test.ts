...
void test("shows time for a session updated today", () => {
  ...
  assert.ok(
    result.includes("10") || result.includes("30"),
    `expected time string, got "${result}"`,
  );
  ...
  assert.ok(
    ...
    `should not be 'Yesterday', got "${result}"`,
  );
});
...
void test("shows AM/PM time for a morning session today", () => {
  ...
  assert.ok(
    /\d{1,2}:\d{2}\s*(AM|PM)/i.test(result),
    `expected time with AM/PM, got "${result}"`,
  );
});
...
void test("shows 'Yesterday' with time for a session updated yesterday evening", () => {
  ...
  assert.ok(
    result.startsWith("Yesterday"),
    `should start with Yesterday, got "${result}"`,
  );
  // Must include a time portion after "Yesterday"
  assert.ok(
    /Yesterday\s+\d{1,2}:\d{2}\s*(AM|PM)/i.test(result),
    `expected 'Yesterday HH:MM AM/PM', got "${result}"`,
  );
});
...
void test("yesterday sessions with different times are distinguishable", () => {
  ...
  assert.notEqual(
    morning,
    evening,
    `morning and evening yesterday sessions should differ: "${morning}" vs "${evening}"`,
  );
});
...
void test("handles midnight boundary (start of today)", () => {
  ...
  assert.ok(
    !result.includes("Yesterday"),
    `midnight today should not be Yesterday, got "${result}"`,
  );
});
