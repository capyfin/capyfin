...
void test("LaunchpadWorkspace subtitle uses text-[14px]", () => {
  const src = readComponent("LaunchpadWorkspace.tsx");
  assert.ok(src.includes("text-[14px]"), "Subtitle should use text-[14px]");
  assert.ok(
...
void test("ActionCardItem title uses text-[15px]", () => {
  ...
  assert.ok(src.includes("text-[15px]"), "Card title should use text-[15px]");
...
void test("typography staircase: heading > subtitle > card title > card desc > section label", () => {
  ...
  const cardDescSize =
    /card\.promise[\s\S]*?text-\[(\d+)px\]/.exec(card) ??
  ...
  assert.ok(
    heading > subtitle,
    `heading (${String(heading)}) must be > subtitle (${String(subtitle)})`,
  );
  assert.ok(
    subtitle < cardTitle,
    `subtitle (${String(subtitle)}) should be < card title (${String(cardTitle)}) — subtitle is lighter weight`,
  );
  assert.ok(
    cardTitle > cardDesc,
    `card title (${String(cardTitle)}) must be > card desc (${String(cardDesc)})`,
  );
  assert.ok(
    cardDesc > sectionLabel,
    `card desc (${String(cardDesc)}) must be > section label (${String(sectionLabel)})`,
  );
});
